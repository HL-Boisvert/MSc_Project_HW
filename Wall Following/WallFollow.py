...
from data_script import Output
...
class WallFollow:
    def __init__(self):
        ...
        self.log_freq = 0.2 # Logging frequency in seconds
        self.odometry = None
        self.imu = None
    ...
    def getMinRange(self, data):
        ranges = []
        for i in range(-30, 30):
            ranges.append(self.getRange(data,i))
        return min(ranges)


    def pidControl(self, error, data):
        global integral
        global prevError
        global kp
        global ki
        ...
        if (datetime.datetime.now() - self.last_update_time).total_seconds() > self.log_freq:
            self.last_update_time = datetime.datetime.now()
            self.out.append(data, angle, self.get_car_linear_velocity())
    ...
    def lidar_callback(self, data):
        ...
        if error is not None:
            self.pidControl(error, data)  # Send the error to pidControl
